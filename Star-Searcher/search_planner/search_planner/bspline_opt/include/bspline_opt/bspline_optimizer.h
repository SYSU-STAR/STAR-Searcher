...
private:
  ...
  void calcYawFeasibilityCost(const vector<Eigen::Vector3d> &q, const double &dt, double &cost, vector<Eigen::Vector3d> &gradient_q);
  bool isQuadratic();
  ...
  double max_yaw_change;      // yaw change limit
  double wnl_, dlmin_;
  ...
  vector<Eigen::Vector3d> g_q_, g_smoothness_, g_distance_, g_feasibility_, g_start_, g_end_, g_guide_,
      g_waypoints_, g_view_, g_time_, g_yaw_feasibility_;
